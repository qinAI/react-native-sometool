...
class RNVideoTrimmer: NSObject {
  ...
  @objc func trim(_ source: String, options: NSDictionary, callback: @escaping RCTResponseSenderBlock) {
      ...
      guard let cachesDirectory = try? manager.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: true)
          else {
      ...
      var outputURL = cachesDirectory.appendingPathComponent("output")
      do {
  ...
  @objc func boomerang(_ source: String, options: NSDictionary, callback: @escaping RCTResponseSenderBlock) {
    ...
    var outputURL = cachesDirectory.appendingPathComponent("output")
    var finalURL = cachesDirectory.appendingPathComponent("output")
    do {
      try manager.createDirectory(at: outputURL, withIntermediateDirectories: true, attributes: nil)
